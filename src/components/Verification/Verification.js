...
const Verification = () => {
  ...
  const verifyOTP = async () => {
    ...
    let payload = {
      ...
      otp: await window.to_electron.enCryptPin(otp),
    };
    setIsSubmit(true);
  ...
  return (
    <div className="verification-container">
      ...
      <div>{disable && <Timer setDisable={setDisable} />}</div>
      <div>
        <button
          disabled={isSubmit || otp.length < 4}
          onClick={() => verifyOTP()}
          className="verification-submit resend"
        >
          Submit
        </button>
        <button
